...
import { RiAddLine, RiCloseLine, RiEditLine, RiDeleteBin2Line, RiMapPinLine } from "react-icons/ri";
...
export function PropiedadesTemplate() {
    ...
    return (
        <Page>
                ...
                <ChipGroup>
                    {["todos", ...TIPOS_PROP.map(t => t.key)].map(k => (
                        <Chip key={k} $active={filtroTipo === k} $color={TIPOS_PROP.find(t => t.key === k)?.color ?? "#94a3b8"}
                            ...
                            {k === "todos" ? "Todos los tipos" : TIPOS_PROP.find(t => t.key === k)?.label}
                        </Chip>
                    ))}
                </ChipGroup>
            </FiltrosWrap>
            ...
            {/* ── Grid de tarjetas ── */}
            {isLoading ? (
                <Vacio>Cargando propiedades...</Vacio>
            ) : visibles.length === 0 ? (
                <Vacio>
                    <Icon icon="solar:home-smile-bold-duotone" style={{ fontSize: 48, color: "rgba(255,255,255,0.1)", display: "block", marginBottom: 12 }} />
                    No hay propiedades{filtroEstado !== "todos" || filtroTipo !== "todos" ? " con ese filtro" : ". Agrega la primera."}
                </Vacio>
            ) : (
                <Grid>
                    {visibles.map((p, i) => {
                        ...
                        return (
                            <PropCard key={p.id} $color={tipo.color} $i={i}>
                                ...
                                <PropActions>
                                    ...
                                    <BtnEliminar onClick={() => confirmarEliminar(p)}>
                                        <RiDeleteBin2Line />
                                    </BtnEliminar>
                                </PropActions>
                            </PropCard>
                ...
                <Overlay onClick={cerrar}>
                    <Modal onClick={e => e.stopPropagation()}>
                        ...
                        <ModalBody>
                            ...
                            <Input value={form.titulo} onChange={e => set("titulo", e.target.value)} placeholder="Ej: Apartamento El Poblado 85m²" />

                            <Row2>
                                <div>
                                    <Label>Precio ($)</Label>
                                    <Input type="number" value={form.precio} onChange={e => set("precio", e.target.value)} placeholder="450000000" />
                                </div>
                                <div>
                                    <Label>Área (m²)</Label>
                                    <Input type="number" value={form.area_m2} onChange={e => set("area_m2", e.target.value)} placeholder="85" />
                                </div>
                            </Row2>
                            ...
                            <Row4>
                                <div>
                                    ...
                                    <Input type="number" min={0} value={form.habitaciones} onChange={e => set("habitaciones", Number(e.target.value))} />
                                </div>
                                <div>
                                    <Label>Baños</Label>
                                    <Input type="number" min={0} value={form.banos} onChange={e => set("banos", Number(e.target.value))} />
                                </div>
                                <div>
                                    <Label>Garajes</Label>
                                    <Input type="number" min={0} value={form.garajes} onChange={e => set("garajes", Number(e.target.value))} />
                                </div>
                                <div>
                                    <Label>Estrato</Label>
                            ...
                            <Row2>
                                <div>
                                    <Label>Ciudad</Label>
                                    <Input value={form.ciudad} onChange={e => set("ciudad", e.target.value)} placeholder="Medellín" />
                                </div>
                                <div>
                                    <Label>Sector / Barrio</Label>
                                    <Input value={form.sector} onChange={e => set("sector", e.target.value)} placeholder="El Poblado" />
                                </div>
                            </Row2>

                            <Label>Dirección</Label>
                            <Input value={form.direccion} onChange={e => set("direccion", e.target.value)} placeholder="Cra 43A # 1 Sur 100" />
...
const PropCard = styled.div`
    ...
    animation:${fadeUp} .4s ${({$i})=>$i*.05}s ease both;
...
`;
const PropNombre = styled.h3`
    font-size:15px;font-weight:800;color:#fff;margin:0;line-height:1.3;
`;
const PropUbicacion = styled.div`
    display:flex;align-items:center;gap:5px;font-size:12px;
    color:rgba(255,255,255,0.4);font-weight:500;
    svg{font-size:14px;flex-shrink:0;}
`;
const PropSpecs = styled.div`display:flex;flex-wrap:wrap;gap:8px;`;
const Spec = styled.span`
    display:flex;align-items:center;gap:4px;font-size:12px;
    font-weight:600;color:rgba(255,255,255,0.55);
    svg{font-size:13px;color:rgba(255,255,255,0.3);}
`;
const PropPrecio = styled.div`
...
`;
const PropActions = styled.div`display:flex;gap:8px;margin-top:4px;`;
...
const ModalTitulo = styled.h2`
    ...
    color:#fff;margin:0;svg{font-size:20px;}
`;
